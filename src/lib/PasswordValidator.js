...
export default class extends BaseValidator {
  ...
  _checkLength() {
    if (this.val.length >= 8) {
      ...
      return Promise.reject(super._errorResult('パスワードが短すぎます。'))
    }
  }
  _checkCapLetter() {
    const re = /[A-Z]+/;
    ...
    if(valid) {
      ...
      return Promise.reject(super._errorResult(`${this.typeName}には大文字を1文字以上含めてください。`))
    }
  }
  _checkSymbol() {
    const re = /[_\.@\-]+/;
    ...
    if(valid) {
      ...
      return Promise.reject(super._errorResult(`${this.typeName}には_.@-を1文字以上含めてください。`))
    }
  }
  _checkFormat() {
    const re = /[a-z0-9]+/;
    ...
    if(valid) {//フォーマットが正しかったら
      ...
      return Promise.reject(super._errorResult(`${this.typeName}は半角英数字の小文字と数字を1文字以上含めてください。`))
    }
  }
